$(document).ready(function () {
  ...
  $("#register_form").on("submit", function () {
    ...
    if (name.val() == "" || name.val().length <= 2) {
      ...
      $("#u_error").html(
        "<span class='text-danger'>Name should be more than 2 char</span>"
      );
      status = false;
    } else {
    ...
    if (pass1.val() == "" || pass1.val().length < 5) {
      ...
      $("#p1_error").html(
        "<span class='text-danger'>Please Enter more than 5 digit password</span>"
      );
      status = false;
    } else {
  ...
  function fetch_brand() {
    $.ajax({
      ...
      data: { getbrand: 1 },
      success: function (data) {
        var chose = "<option value='0'>Choose Batch</option>";
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data == "catagory_added") {
            ...
            $("#cat_error").html(
              "<span class='text-success'>New category Added</span>"
            );
            $("#catagory_name").val("");
            fetch_category();
          } else {
  ...
  $("#brand_form").on("submit", function () {
    if ($("#brand_name").val() == "") {
      ...
      $("#brand_error").html(
        "<span class='text-danger'>Add A Brand Name</span>"
      );
    } else {
      $.ajax({
        url: DOMAIN + "/includes/process.php",
        ...
        success: function (data) {
          if (data == "brand_added") {
            ...
            $("#brand_error").html(
              "<span class='text-success'>New Brand Added</span>"
            );
            $("#brand_name").val("");
            fetch_brand();
          } else {
